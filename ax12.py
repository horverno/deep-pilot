...
class Ax12:
    ...
    AX_GOAL_POSITION_L = 30 #0x1E
    AX_GOAL_POSITION_H = 31 #0x1F
    AX_GOAL_SPEED_L = 32    #
    AX_GOAL_SPEED_H = 33
    ...
    def __init__(self):
        if(Ax12.port == None):
            Ax12.port = Serial("/dev/ttyS0", baudrate=1000000, timeout=0.001) #1000000 0.001
        if(not Ax12.gpioSet):
    ...
    def direction(self,d):
        ...
        '''if d == Ax12.RPI_DIRECTION_RX:
            sleep(0.00001) # Ax12.RPI_DIRECTION_SWITCH_DELAY
        else:
            sleep(0.01)
        '''
    ...
    def readData(self,id):
        ...
        #print(reply) # b'x\ff\xff\x0f\x02\x00
        #for c in reply:
        #    print("%2x " % ord(c), end="")
        #print("")
    ...
    def setBaudRate(self, id, baudRate):
        self.direction(Ax12.RPI_DIRECTION_TX)
        Ax12.port.flushInput()
        br = ((2000000/int(baudRate))-1) # python 2.7 long
    ...
    def setMovingSpeed(self, id, speed):
        ...
        Ax12.port.flushInput()
        s = [speed&0xff, speed>>8]
        checksum = (~(id + 5 + Ax12.AX_WRITE_DATA + Ax12.AX_GOAL_SPEED_L + s[0] + s[1]))&0xff
        outData = bytes([Ax12.AX_START])
        outData += bytes([Ax12.AX_START])
        outData += bytes([id])
        outData += bytes([5])
        outData += bytes([Ax12.AX_WRITE_DATA])
        outData += bytes([32]) #Ax12.AX_GOAL_SPEED_L
        outData += bytes([s[0]])
        outData += bytes([s[1]])
        outData += bytes([checksum])
        Ax12.port.write(outData)
